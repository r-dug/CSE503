...
template <typename Comparable, typename Iterator, typename Comparator>
void insertionSort(vector<Comparable> &a, Iterator begin, Iterator end, Comparator lessThan)
{
	
	if (begin != end)
		insertionSortHelp(a, begin, end, lessThan);
}
...
int main()
{
	...
	insertionSort(v1, v1.begin(), v1.end(), less<int>());
	initVSame(v1);
	...
	printMyList(v1);
	insertionSort(v1, v1.begin(), v1.end(), less<int>());
	cout << "it would appear our run time is O(N-1) if all elements are the same.... pretty good!\n\n";
